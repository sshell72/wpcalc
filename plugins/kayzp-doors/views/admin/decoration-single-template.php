...
<form class="street-container" method="POST" enctype="multipart/form-data">
	...
	<?php if(empty($_GET['decor_id'])) { ?>
	<a href="#" class="inherit" data-popup="inherit" data-decor="<?php echo $decor_id; ?>">Унаследовать все настройки</a><?php } ?>
...
"
			...
			<select  class="" disabled id="params_vis" name="params_vis[]" multiple>             
					<?php
        ...
        <div class="item-base" data-locked-id="" data-decor-id="<?php echo $decor_id; ?>">
            ...
            <select  class=""  name="choose-base-param" data-decor-id="<?php echo $decor_id; ?>">
            ...
            <span>и цена параметров</span>
				<select  class="" id="params" name="params[]" multiple>             
					<?php
						foreach ($parameters_decor as $parameter_decor) {
							?>
							<option  value="<?php echo $parameter_decor->id; ?>">
            ...
            <button class="add_rule click">Сохранить</button>
        </div>
            ...
            if(!empty($parameters)){
                foreach ($parameters as $parameter) {
                    ...
                    if (!empty($parameter_in_decor_table)) {
                     ...
                     //$param_visibles = $wpdb->get_results("SELECT * FROM {$table_visible_param} WHERE param_id={$parameter->id} AND decor_id={$decor_id}");
                    ...
                    $values_available_all = $wpdb->get_results("SELECT available_decor_values,text_def FROM {$table_available_param} WHERE decor_id={$decor_id} AND param_id={$parameter->param_id}");
                    ...
                    ?>
                        <tr data-id="<?php echo $param->id; ?>" data-dec-id="<?php echo $decor_id; ?>">
							...
							<td>
							<?php if(isset($parameter_in_decor_table->description)) $pidt = $parameter_in_decor_table->description; else $pidt = ''; ?>
                                <textarea class="description" name="description" value="<?php echo $pidt; ?>"><?php echo $pidt; ?></textarea>
                            </td>
                            <td>
                            ...
                            <td>
                                <?php if(strripos($parameter_type->type, 'список') !== false || strripos($parameter_type->type, 'выбор') !== false) { ?>
                                    <select class="" name="default-param[]" id="" multiple>
                                        ...
                                        if(!empty($values_available_all)){
                                            ...
                                            foreach ($values_available_all as $values_available) {
                                                ...
                                                $item_def = $wpdb->get_row("SELECT * FROM {$table_available_param} WHERE param_id={$param->id} AND decor_id={$decor_id}");
													...
													$def_values = json_decode($item_def->def_value);
													if(is_array($def_values)){
														foreach($def_values as $def_value){
															if($def_value == $item->id)
															$selected = 'selected';
														}
													}
                                ...
                                <?php }else{
										?>
										<?php if(strripos($parameter_type->type, 'поле') !== false) {
											$text_def = '';
											if(isset($values_available_all[0]->text_def) && $values_available_all[0]->text_def != 0) $text_def = $values_available_all[0]->text_def;?>
										<input type="text" class="default-text" name="default-text" value="<?php  echo $text_def; ?>">
										<?php } }	?>
                            </td>
                            <td>
                                <?php
                                if (strripos($parameter_type->type, 'список') !== false || strripos($parameter_type->type, 'выбор') !== false) : ?>
                                <?php
                                    if(false){
                                    foreach ($values_available_all as $values_available) {
                                ...
                                <select class="" name="available-param[]" multiple>
                                    ...
                                    if(!empty($parameter->available_values)){
                                        ...
                                        foreach ($values as $value) {
                                            ...
                                            ?>
                                            <option <?php echo $checked; ?> value="<?php echo $value; ?>"><?php 
...
"
        ...
        <div class="item-block" data-locked-id="" data-decor-id="<?php echo $decor_id; ?>">
            ...
            <select class="" name="choose-bloc-param" data-decor-id="<?php echo $decor_id; ?>">
                <option value="">Выберите параметр</option>
                <?php
                if(!empty($parameters)) {
                    foreach ($parameters as $parameter) {
                        ...
                        if($param->type_id != 2 && $param->type_id != 3 && $param->type_id != 5) continue;
                        ?>
            ...
            <select class="" name="values-block" data-series_id="<?php echo $model_field->series_id; ?>">
                <option value="">Выберите значение</option>
            </select>

            <span>, то заблокировать поле</span>
            <select class="" name="blocked-param" data-decor-id="<?php echo $decor_id; ?>">
                <option value="0
">Выберите поле</option>
                <?php
                if(!empty($parameters)) {
                    foreach ($parameters as $parameter) {
                        if($parameter->type_id != 2 && $parameter->type_id != 3 && $parameter->type_id != 5) continue;
                        ?>
            ...
            <button class="add_rule clickeds">Сохранить</button>
        </div>
    ...
    <div class="container-coefficient" data-decor-id="<?php echo $decor_id; ?>">
        <?php 
		if(isset($model_field)){
			$series_id = $model_field->series_id;
			$params = $wpdb->get_results("SELECT * FROM wp_doors_params_series WHERE series_id={$series_id}");
            $blocked_params = $wpdb->get_results("SELECT * FROM wp_doors_price_coefficient WHERE decor_id={$decor_id}");
            foreach ($blocked_params as $blocked_param): 
        ?>
            <div class="item-coefficient" style="margin-bottom: 20px;" data-locked-id="<?php echo $blocked_param->id ?>" data-decor-id="<?php echo $decor_id; ?>">
            <span>Если параметр</span>
            "
            <?php 
                $param= $wpdb->get_row("SELECT * FROM wp_doors_params WHERE id={$blocked_param->param_id}");
...
"
            ...
            ?>
"
            <span>, то изменить стоимость поля</span>"
            
            <?php $result = '';
					if($blocked_param->param_coefficient_id){
						$par = json_decode($blocked_param->param_coefficient_id);
						$par = implode(',',$par);

							$parametrs = $wpdb->get_results("SELECT * FROM wp_doors_params WHERE id IN ({$par})");
							...
							?>
                    <select  class="" disabled id="params_vis_price" name="params_vis_price[]" multiple>             
					<?php
				...
				"
            <span> на</span>"
               <?php echo $blocked_param->coefficient ?>
            "
            <span class="delete_row kayzp_del_price" style="margin: 0 15px;">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="15px" width="15px" x="0px" y="0px"
                     viewBox="0 0 486.4 486.4" style="enable-background:new 0 0 486.4 486.4;" xml:space="preserve">
                    <g>
                        <g>
        ...
        <div class="item-coefficient" data-locked-id="" data-decor-id="<?php echo $decor_id; ?>">
            <span>Если параметр</span>

            <select class="" name="choose-coefficient-param" data-decor-id="<?php echo $decor_id; ?>">
                <option value="">Выберите параметр</option>
                <?php
                if(!empty($parameters)) {
                    foreach ($parameters as $parameter) {
            ...
            <select class="" name="values-coefficient" data-series_id="<?php echo $model_field->series_id; ?>">
                <option value="">Выберите значение</option>
            </select>

            <span>, то изменить стоимость поля</span>
            <select class="" id="coefficient-param" name="coefficient-param[]" data-decor-id="<?php echo $decor_id; ?>" multiple>
				...
				if ($params) {
					foreach ($params as $param) {
						$parametr = $wpdb->get_row("SELECT p.id, p.name FROM wp_doors_params p JOIN wp_doors_value_p v ON p.id={$param->param_id} AND p.id=v.param_id");
						if ($parametr) {
			 ...
			 <span>на</span>
			 <input type="text" name="coefficient_price" value="">
...
<div class="popup_inherit" id="inherit">
    <form class="content_popup inherit_go" action="#" method="post">
        <h2>Унаследовать все настройки отделки</h2>
        <span class="close_popup">&times </span>
        <select class="" id="parent-inherit" name="id_decor" >
		<option value="">Выберите отделку</option>
		   <?php
			$result = '';

			$parametrs = $wpdb->get_results("SELECT p.id as pi, p.name as pn, v.name as vn  FROM wp_doors_decoration p JOIN wp_doors_models v ON p.visible=1 AND p.model_id=v.id");
			foreach($parametrs as $parametr) {
				$result .= '<option value="'.$parametr->pi.'">'.$parametr->pn.'|'.$parametr->vn.'</option>';
			}
        ...
        <input type="submit" value="Наследовать" class="blue-button-doors">
    </form>
</div>