...
<form class="street-container" method="POST">
    ...
    <h1>Модели</h1>
	<a href="#" class="copy" data-popup="copy" >Скопировать все настройки модели</a>
    <table>
        <thead>
        <tr>
            <th>Публикация</th>
                ...
                foreach ($models as $model){
                    ...
                    if(is_array($decorations))
                    {
                        foreach ($decorations as $decoration) {
                            $decoration_str .= '<div class="'.$decoration->id.'"><a href="admin.php?page=parameters_decor&model_id='.$model->id.'&decor_id='.$decoration->id.'">' . $decoration->name . '</a>
                                                <span data-model="'. $model->id .'" data-decor="'.$decoration->id.'" class="delete_decor">
...
<div class="popup_copy" id="copy">
    <form id="popup-copy" class="content_popup copy_go" action="#" method="post">
        <h4>Выберите модель для копирования</h4>
        <span class="close_popup">&times </span>
		
        <span>Все настройки будут скопированы с отделки:</span>
        <select class="" id="parent-copy" name="id_decor_parent" >
		<option value="">Выберите отделку</option>
		   <?php
			$result = '';

			$parametrs = $wpdb->get_results("SELECT v.series_id as si, p.id as pi, p.name as pn, v.name as vn  FROM wp_doors_decoration p JOIN wp_doors_models v ON p.visible=1 AND p.model_id=v.id");
		...
		<span>и применены к отделке:</span>
        <select class="" id="childe-copy" size="16" name="id_decor_childe[]" multiple>
		   <option value="">Выберите параметр выше</option>
		</select>
