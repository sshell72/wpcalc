...
class DoorsFrontendclass{
    ...
    public static function frontend_init_hooks() {
        ...
        add_action('wp_ajax_new_coefficient_price',[__CLASS__,'new_coefficient_price']);
        add_action('wp_ajax_nopriv_new_coefficient_price',[__CLASS__,'new_coefficient_price']);
...
//    }
    ...
    public function submit_series(){
		global $wpdb;
        $series =  $wpdb->get_row('SELECT description FROM wp_doors_series where id = '.$_POST['id']);
		$series_html = '<div class="serdiv"><span class="serspan">'.$series->description.'</span></div>';
...
//            $param_values =  self::get_query('SELECT id_value FROM wp_doors_param_values where id_param = '.$parameters[$i]);
    ...
    public function submit_decoration(){
		...
		$coefficients = $wpdb->get_results("SELECT * FROM wp_doors_price_coefficient where decor_id=".$_POST['decoration_id']);
        $q = $wpdb->get_row("SELECT * FROM wp_doors_decoration where id=".$_POST['decoration_id']);
        if ($q) {
			$coef = $wpdb->get_row("SELECT s.coeff c FROM wp_doors_series s INNER JOIN wp_doors_models m INNER JOIN wp_doors_decoration d WHERE s.id=m.series_id AND m.id=d.model_id AND d.id = ".$_POST['decoration_id']);
            $result['image'] = $q->photo_front;
            $result['back_image'] = $q->photo_back;
            $result['dec_name'] = $q->name;
            $result['dec_price'] = $q->price;
            $result['series_coeff'] = $coef->c;
            ...
            foreach ($q as $a_param) {
                ...
                if ($param) {
                    ...
                    $fields_values = array();
                    if ($type->id == 2 || $type->id == 3 || $type->id == 5) {
                    ...
                    $field_html = '';
					 ...
					 $description = ($a_param->description != '') ? '<div class="block_cont"> 
									 <a href="#" class="foc" onclick="return false"><i class="fa fa-comment-o" aria-hidden="true"></i></a> 
									 <span class="hid">'.$a_param->description.'</span> 
									 </div>' : '';
                    ...
                    switch ($type->id) {
                        case '1':
                            ...
                            $field = '
                                <div id="'.$param->id.'" class="bl input-group '.$col.'">
                                    <div class="input-group-addon">'.$param->name.':</div>'.$description.'
									
                                    <input type="text" name="field-'.$param->id.'" class="form-control" id="'.$param->name.'" value="'.$a_param->text_def.'" />
                                </div>
                        ...
                        case '2':
                            ...
                            $field = '
                                <div id="'.$param->id.'" class="bl '.$col.'">
									...
									'.$description.'
                                    <select class="selectpicker selectp" name="field-'.$param->id.'" data-size="5" data-container="body">
                            ...
                            foreach ($fields_values as $option) {
								...
								foreach ($coefficients as $coefficient) {
									$coefficient_data = $coefficient->coefficient;
									$param_coefficient_id = json_decode($coefficient->param_coefficient_id);
									$param_coefficient_id = implode(",", $param_coefficient_id);
									if($coefficient->value_id == $option['id']) {
										$coeff .= $param_coefficient_id.';';
										$coeff_data .= $coefficient_data.';';
									}
								}
								$coeff = rtrim ($coeff , ";");
								$coeff_data = rtrim ($coeff_data , ";");
								...
								$block = rtrim ($block , ",");
                                $field .= '<option '.$selected.' value="'.$option['name'].'" data-blocked="'.$block.'" data-coefficient="'.$coeff.'" data-coefficient_data="'.$coeff_data.'" data-price="'.$option['price'].'">'.$option['name'].'</option>';
                            }
						...
						case '5':
                            ...
                            $field = '
                                <div id="'.$param->id.'" class="bl '.$col.'">
                                    <label>'.$param->name.'</label>
									'.$description.'
                                    <select class="selectpicker selectp" size="5" name="field-'.$param->id.'" data-size="5" data-container="body" multiple>
                                        ';

                            foreach ($fields_values as $option) {
                                $selected = '';
                                
								if($a_param->def_value != ''){
									foreach (json_decode($a_param->def_value)  as $def_value) {
										if ($option['id'] == $def_value) {
										   $selected = 'selected';
										}
									}
								}
                       ...
                       case '3':
							...
							<div id="'.$param->id.'" class="bl '.$col.'">
                                <label>'.$param->name.':</label>
								'.$description.'
                                <select id="gridn" name="field-'.$param->id.'" class="selectpicker color selectp" data-size="5" data-container="body">
                                    <option>Выберите значение</option>';
                                    ob_start();
                            ...
                            foreach ($fields_values as $option) {
								...
								$block = rtrim ($block , ",");
                                $field .= '<option '.$selected.' value="'.$option['name'].'" data-blocked="'.$block.'" data-coefficient="'.$coeff.'" data-coefficient_data="'.$coeff_data.'" data-subtext="'.$name[0].'" data-price="'.$option['price'].'">'.$name[1].'</option>';
                            }
                            $field .= '    </select>
							</div>
                            ';
			...
			$opt_name = $wpdb->get_row('SELECT * FROM `wp_doors_base` WHERE `decor_id`= '.$decoration_id);
            ...
            $result['fields_html'][] = '
				<input type="hidden" name="opt_name" value="'.$opt_name->opt.'">
                <p>
