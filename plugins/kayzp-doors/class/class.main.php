...
class DoorsMainclass{
   ...
   //  Active plugin
    ...
    public static function doc_init_hooks() {
        ...
        add_action( 'wp_ajax_delete_decor', [__CLASS__, 'delete_decor'] );
        add_action( 'wp_ajax_delete_rule_coefficient', [__CLASS__, 'delete_rule_coefficient'] );
        add_action( 'wp_ajax_delete_rule', [__CLASS__, 'delete_rule'] );
        add_action( 'wp_ajax_delete_base', [__CLASS__, 'delete_base'] );
        ...
        add_action( 'wp_ajax_decor_parent', [__CLASS__, 'decor_parent'] );
        add_action( 'wp_ajax_get_values_for_coefficienting', [__CLASS__, 'get_values_for_coefficienting'] );
        add_action( 'wp_ajax_get_fields_for_blocking', [__CLASS__, 'get_fields_for_blocking'] );
        add_action( 'wp_ajax_get_fields_for_coefficienting', [__CLASS__, 'get_fields_for_coefficienting'] );
        ...
        add_action( 'wp_ajax_redact_parameters', [__CLASS__, 'redact_parameters'] );
        ...
        add_action( 'wp_ajax_add_def_text', [__CLASS__, 'add_def_text'] );
        add_action( 'wp_ajax_get_values_from_block_param', [__CLASS__, 'get_values_from_block_param'] );
        add_action( 'wp_ajax_get_values_from_coefficient_param', [__CLASS__, 'get_values_from_coefficient_param'] );
        add_action( 'wp_ajax_add_block_param', [__CLASS__, 'add_block_param'] );
        add_action( 'wp_ajax_add_coefficient_param', [__CLASS__, 'add_coefficient_param'] );
        add_action( 'wp_ajax_add_base_param', [__CLASS__, 'add_base_param'] );
...
dashicons-admin-home' );
		...
		add_menu_page( 'Сервис', 'Сервис', 'manage_options', 'service_parameters', [__CLASS__, 'service_parameters_show'], 'dashicons-editor-spellcheck' );
    }
    ...
    public static function door_series_single_show() {
        ...
        if(isset($_POST['series_id']) && $_POST['series_id'] != 0){
            ...
            $coeff = isset($_POST['coeff']) ? $_POST['coeff'] : 1;
			
            $description = isset($_POST['description']) ? $_POST['description'] : '';

            $result = $wpdb->update(
                $table_series,
                [ 'name' => wp_unslash( $_POST['name']), 'visible' => $show, 'coeff' => $coeff, 'description' => $description ],
                [ 'id' => $_POST['series_id'] ]
            );
            echo (!empty($result)) ? '<p class="success-db">Серия успешно обновлена</p>' : '<p class="error-db">Ошибка сохранения1</p>';
        }
        elseif (isset($_POST['category_id'])) {
            ...
            $result = $wpdb->insert(
                $table_series,
                [ 'name' => wp_unslash($_POST['name']), 'visible' => $show, 'category_id' => $category_id, 'coeff' => $coeff, 'description' => $description  ],
                [ '%s', '%d' ]
            );

            echo (!empty($result)) ? '<p class="success-db">Серия успешно сохранена</p>' : '<p class="error-db">Ошибка сохранения2</p>';
    ...
    //  Show Service
    public static function service_parameters_show() {
        self::doc_view('admin/service');
    }
    ...
    public static function door_parameters_single_show() {
        if (isset($_POST['category_id'])) {
            ...
            echo (!empty($result)) ? '<p class="success-db">Серия успешно сохранена</p>' : '<p class="error-db">Ошибка сохранения3</p>';
    ...
    public static function handlersSeries() {
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                $('.limit-params.dec tr input, .limit-params.dec tr select, .limit-params.dec tr textarea').on('change', function(){
                ...
                // Delete row
                $('.delete_row:not(.kayzp_del, .ka_del, .kayzp_del_price)').on('click', function () {
                    if(confirm('Удалить?')){
				...
				$('.param-pop-redact').on('submit', function (e) {
                    e.preventDefault();
                ...
                // Add new block param
                $(document).on('click', '.item-block button.clickeds', function (e) {
                    e.preventDefault();
    ...
    public static function add_coefficient_param() {
        $id_param = $_POST['id_param'];
        $id_value = $_POST['id_value'];
        $id_param_coefficient = json_encode($_POST['id_param_coefficient']);
        $id_decor = $_POST['id_decor'];
        $coefficient = $_POST['coefficient'];

        global $wpdb;

        $table_coefficient = $wpdb->get_blog_prefix() . 'doors_price_coefficient';
        $wpdb->insert(
            $table_coefficient,
            [ 'decor_id' => $id_decor, 'param_id' => $id_param, 'value_id' => $id_value, 'param_coefficient_id' => $id_param_coefficient, 'coefficient' => $coefficient ],
            [ '%s', '%d' ]
        );
        exit(json_encode('Сохранено!'));
    }

    public static function add_base_param() {
		$decor_id = $_POST['decor_id'];
        $opt = $_POST['opt'];
        $id_value1 = $_POST['id_value1'];
        $id_value2 = $_POST['id_value2'];
        $price = $_POST['price'];
		...
		$param = $wpdb->get_row("SELECT * FROM wp_doors_base where opt='".$opt."' and decor_id=".$decor_id." and ((val1<=".$id_value1." and val2>=".$id_value1.") or (val1<=".$id_value2." and val2>=".$id_value2.") or (val1>=".$id_value1." and val2<=".$id_value2."))");
		
		if (!$param) {

        $table_blocked = $wpdb->get_blog_prefix() . 'doors_base';
        $wpdb->insert(
            $table_blocked,
            [ 'opt' => $opt, 'decor_id' => $decor_id, 'val1' => $id_value1, 'val2' => $id_value2, 'price' => $price, 'params' => $params, 'coefficient' => $coefficient ],
            [ '%s', '%d' ]
        );
        exit(print_r('Сохранено!'));
		}else{
			exit(print_r('Ошибка! Неверные параметры размеров! Такие размеры уже есть.'));
		}
    }

    public static function get_values_from_coefficient_param() {
 ...
 */
   ...
   public static function add_def_text() {
    ...
    public static function redact_parameters() {
        global $wpdb;
    ...
    public static function delete_decor() {
        global $wpdb;
        ...
        $table_name = $wpdb->get_blog_prefix() . 'doors_decoration';
            ...
            $result = $wpdb->delete( $table_name, ['id' => $decor_id] );

        exit(json_encode($result));
    }

    public static function delete_base() {
        global $wpdb;

        $id = $_POST['id'];

        $table_name = $wpdb->get_blog_prefix() . 'doors_base';
    ...
    public static function delete_rule_coefficient() {
        global $wpdb;

        $id = $_POST['id'];

        $table_name = $wpdb->get_blog_prefix() . 'doors_price_coefficient';
...
public static function delete_rule() {
    ...
    public static function get_values_for_coefficienting() {
        global $wpdb;
    ...
    public static function decor_parent() {
        global $wpdb;
		...
		$parametrs = $wpdb->get_results("SELECT v.series_id as si, p.id as pi, p.name as pn, v.name as vn  FROM wp_doors_decoration p JOIN wp_doors_models v ON v.series_id = {$series_id} AND p.model_id=v.id");
		foreach($parametrs as $parametr) {
			$result .= '<option data-series="'.$parametr->si.'" value="'.$parametr->pi.'">'.$parametr->pn.'|'.$parametr->vn.'|'.$parametr->si.'</option>';
		}
    ...
    public static function showParameterRow($parameters) {
        ...
        foreach ($parameters as $param) {
            ...
            $table_name = $wpdb->get_blog_prefix() . 'doors_block_params';
            $name_block_param = $wpdb->get_row( "SELECT block FROM {$table_name} WHERE id={$param->block_id}" )->block;
            ?>
            <tr data-id="<?php echo $param->id ?>">
                <td class="name-param-text" data-name-param="<?php echo $param->type_id; ?>">
                    <?php echo $param->name; ?>
                </td>
                <td class="name-series type-param" data-type-param="<?php echo $param->type_id; ?>">
                    <?php echo $name_type_param; ?>
                </td>
                <td class="name-series block-param" data-block-param="<?php echo $param->block_id; ?>">
                    <?php echo $name_block_param; ?>
                </td>
                <td>
                    <?php if($param->type_id == 2 || $param->type_id == 3 || $param->type_id == 5) : ?>
                        <a href="admin.php?page=parameters_single&param_id=<?php echo $param->id; ?>">Редактировать</a>
                    <?php elseif($param->type_id == 4 || $param->type_id == 5) : ?>
                ...
                <td>
                        <a class="open-redact" href="#"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
